...
[53] **Testing strategy: three registers + the self-testing substrate** - Testing Karto splits into three registers with different tools, semantics, and oracles; conflating them causes either over-testing the wrong layer or missing the load-bearing properties.

**Register 1 — Contract/plumbing** (pytest, deterministic, in `tests/`): validates the framework's own code. Two structural properties serve as oracles without external ground truth — (a) *completeness*: every `ScoreResult` has exactly one `FeatureTrace` per feature declared in `SSP_FEATURES` [48]; (b) *legality*: every emitted feature value is in the declared value space. Additional contract tests: divergence categorization (rules-wins / catboost-wins / all-agree / all-disagree → exact category from [46]), validation rejection messages name the broken feature/position (agent-readable error quality is load-bearing because agents act on these messages), loader idempotency (repeated imports don't collide or reuse stale state). Do not assert on semantic content — whether a value is *correct* for a given input is a domain question, not a plumbing question.

**Register 2 — SSP quality** (per-SSP, the five mirror modes from [42] are the test framework): the declared value space plus the SSP's own fixture inputs are the oracle; no external annotator required. Minimal viable form: a handful of curated inputs → expected positions (regression against the SSP's own declarations) plus the completeness and legality properties above. Full form: kNN purity (k nearest feature-bag neighbors share a cluster), cluster coherence (semantic vs structural assignment agreement rate), dual-substrate agreement rate on a held-out corpus, CatBoost-vs-PCA loading disagreement per [43]. Critical semantic distinction: high dual-substrate disagreement is **not a CI failure** — it is a signal the SSP needs refinement, consumed by the triage loop [41]. The line between registers: if the SSP declares an illegal value or omits a feature, that is a framework bug (Register 1 catches it); if the SSP assigns a semantically wrong value to a specific input, that is a domain disagreement that feeds triage, not CI.

**Register 3 — Command boundary** (smoke tests only): agent-mediated slash commands produce LLM output that is intentionally non-deterministic — do not pin it. Test the Python boundary: command calls the right library functions with the right arguments; CLI exits 0 on valid input and non-zero on invalid; output JSON has the correct top-level shape (`result`, `trace`, `divergence` present). Nothing deeper.

**Cross-register: dual substrate as self-testing mechanism** — the dual-substrate [46] provides an unlabeled quality score requiring no external GT: disagreement rate on a held-out corpus is a standing health metric for any operationalized SSP. Single-substrate SSPs lose this property entirely and should generate a hard warning from the framework. The forward-pass trace [49] is the diagnostic artifact bridging Register 1 and Register 2: when contract tests pass but a domain disagreement surfaces, the trace must identify which feature drove the substrate split — trace diagnostic quality (does the trace localize the divergence to a specific feature?) is itself a Register 1 contract test. The recursive Karto pattern [49][50] extends Register 2 upward: traces become a corpus, meta-SSPs over traces are themselves testable by the same five mirror modes, same framework at the next level.
...
- 2026-05-04: Implementation-design session refined the methodology toward a buildable plugin — added [46]–[52]. Testing strategy added as [53]: three-register split (contract/plumbing / SSP quality / command boundary), dual-substrate as self-testing mechanism, divergence rate as quality metric not CI failure, five mirror modes [42] as the SSP quality test framework, trace diagnostic quality as a Register 1 contract property bridging to [49]. Dual-substrate by default with frontier-LLM tiebreaker cascade and 4-way comparison matrix (sharpens [39]); SSP_META domain vocabulary + descriptive/predictive split (sharpens [38]/[41]); SSP as Python module as canonical form with 200-500-line size sweet spot (resolves the JSON-vs-substrate config tension and [25]'s open question on where SSPs live); forward-pass trace as first-class artifact enabling recursive Karto (meta-SSPs over traces); Karto's own framing using Intent/Data/Compute/Confidence demonstrating fractal application + dogfood-on-p8 V1 acceptance criterion; segmentation as part of framing with 5th triage bucket (segmentation gap) + ingestion pipeline split (plugin adapters + per-project ingestion.py); three registers of LLM-facing instruction (process-level skills / per-SSP prompts / agent-level explanations) with distinct lifecycles. V1 implementation plan drafted (8 tasks, see .agent/tasks/).
